...
async function verifyF19() {
  ...
  console.log(users.map((u: any) => u.name));
  const leak = users.some((u: any) => u.name?.includes("Organization B") || u.name?.includes("Organization C"));
...
async function verifyF11() {
  ...
  const activeDefs = (defs.body?.items ?? defs.body?.workflows ?? []).filter((d: any) => d.active && d.triggerEventType === "lead.created");
