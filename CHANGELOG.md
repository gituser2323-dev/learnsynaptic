...
## Configuration & Integration Verification (post-RC-10)

**Goal:** not a feature pass — verify that every one of the 20 cataloged integrations can actually be configured by a non-technical business owner before Customer #1, and that the app tells the truth about whether each one is working rather than assuming a saved credential is a valid one. New `CONFIGURATION_LAUNCH_CHECKLIST.md` is this pass's primary deliverable.

**The one real gap found**: every integration category except Calendar ("Sync now") and Notification webhooks ("Test Notification") had no way to verify a credential actually works — only whether it was saved. Closed for every remaining category (AI, Storage, Payments, Email, WhatsApp) with a new, real, per-category connection test: `lib/services/integrations/connectionTest.ts` + `POST /api/admin/integrations/[providerId]/test-connection`, wired into a new "Test Connection" button on every applicable provider card in Settings → Integrations. Every branch makes a real, read-only vendor call (a 5-token AI completion, an S3 `HeadBucketCommand`, Cloudinary's account Usage API, Postmark's `GET /server`, a Meta Graph API phone-number lookup, a payment provider's `getPaymentStatus` against a synthetic order id) — never a fabricated pass from an env var merely being present.

**A second, smaller real gap found and closed along the way**: `integrationService.recordSync()` unconditionally no-opped for every `builtIn` provider (WhatsApp/Email/OpenAI/Anthropic/Gemini) — meaning even a real connection test for these could never write to that provider's own Logs panel. Now always logs the real outcome for every provider; only the connection-row health-field update (which these providers don't have, absent a tenant override) stays conditional.

**A third gap found**: `preflightService.ts`'s already-real per-integration configuration report (`tenantIntegrations` — WhatsApp/Email/AI/three payment gateways/three calendar OAuth apps) was computed on every Platform Dashboard load but never rendered anywhere. Added a "Configuration & Integration Verification" panel to the Platform Console dashboard displaying it, plus a new "Webhooks (Outbound Delivery)" entry — the one category this phase's mission named explicitly that had no dedicated line.

**Everything else the mission asked for already existed and needed verification, not construction**: the Integrations Registry already tracked Connected/Disconnected/Health/Last-Sync/Last-Error with real encrypted per-tenant credential storage; `.env.example` was already a complete, correctly-categorized (REQUIRED/RECOMMENDED/PLATFORM/TENANT-AWARE/PUBLIC/OPTIONAL) environment-variable reference; the Startup Health Check the mission asked for was already built (RC-4's `preflightService.ts`, covering Database/Authentication/Encryption/Queue/Cron/Workers/Storage/Observability), just under-surfaced.

**Real configuration test pass, live against this deployment's own (intentionally unconfigured) environment**: every new Test Connection button was exercised for real and returned a correct, honest "not configured" result for every vendor — no false positives. The route's rate limit (10/min, matching the existing `notification-test` convention) was live-confirmed under a rapid sweep across providers.

**Regression**: `tsc` clean, `eslint` clean on every file touched, `vitest run` 834/834 passing (both the pre-existing `integrationService.unit.test.ts` and `preflightService.unit.test.ts` suites unmodified and still green), `next build` clean.

---

## RC-10 — V1.0 Final Production Launch Gate (Production Hardening Release Candidate)

**Goal:** the mission's own words — determine whether LearnSynaptic Business OS can safely onboard its first real, paying external SaaS customer, with a final verdict of exactly GO, CONDITIONAL GO, or NO-GO. Primarily an audit and launch-gate phase, not a feature-development pass. New `RC_10_AUDIT.md` (34-section detailed record) is this pass's primary deliverable. RC-1 through RC-9 were complete before this pass began.

**RC-9's closure claims were independently re-derived, not trusted.** `tsc`/`vitest`/`next build` were all re-run from a clean state (834/834 tests, both clean). Both of RC-9's CRITICAL findings were re-verified live rather than taken on faith: a fresh public lead was created through the real, unauthenticated endpoint and a direct MongoDB query confirmed zero `WorkflowRun` documents were created for it and zero orphaned automation runs exist system-wide (the cross-tenant automation fix); `GET /api/admin/users` as one organization's own admin was re-called and confirmed to return only that organization's own three staff members, zero cross-tenant records (the staff-directory leak fix). Fresh live spot-checks not previously run this exact way — unauthenticated request, cross-tenant read, platform route as tenant admin, RBAC violation — all returned exactly the expected rejection.

**A genuinely new, real finding this pass**: `POST /api/admin/billing/subscription/assign-plan` is a correctly-built, safe, tenant-self-service plan upgrade/downgrade route — but a full search of the admin frontend found zero code anywhere that calls it. The backend capability for a customer to upgrade their own plan is real; the UI for it does not exist. Today, a plan change requires the platform owner to act through the Platform Console. Found by tracing the real first-customer workflow end-to-end in a live browser rather than only checking API surfaces in isolation.

**The single most consequential finding of this pass, carried from `DR_RUNBOOK.md`'s own existing, unprompted disclosure and independently reconfirmed still true**: no real MongoDB Atlas cluster (or equivalent managed, backed-up database) has ever been provisioned for this project — `MONGODB_URI` still points at a local, unmanaged instance. The runbook's own words, unchanged since RC-5, call this "the single highest-severity item in this document." This is not a code defect — the application itself has zero open Critical/High/Medium findings — but it means real customer data would currently have no managed backup or recovery story at all, a hard blocker for safely accepting it.

**A second real, concrete gap found by simply following the real signup form's own consent checkbox to its destination**: "I agree to the Terms of Service and Privacy Policy" links to `/privacy` and `/terms`, both of which return a live 404. The consent mechanism and links are correctly built; the actual legal page content was never written. Per this mission's own explicit instruction, no legal content was authored or implied as reviewed by this audit — the gap is identified for real business/legal review, not fixed in code.

**Every other disclosed gap from RC-1 through RC-9 was independently re-checked against the live codebase** (Conversations RBAC still Admin-only — its real functional impact was newly confirmed live this pass by opening the actual Conversations inbox, not just checking a role string; customer-visible request IDs still never surfaced; support impersonation and platform announcements still correctly unbuilt; admin-assisted password/MFA recovery for another user still absent, though genuine self-service fallbacks — recovery codes, an email-OTP path — mitigate the common cases; encryption-key rotation tooling still absent, though `lib/startupValidation.ts` already prevents the far more dangerous "silently running on a known dev-fallback key" scenario with a loud production-boot error; the RC-7-disclosed leftover test-plan-visible-to-customers issue is confirmed already fixed). A fresh, positive finding along the way: a real, valuable, tenant-scoped Reliability dashboard (queue depth, DLQ, failure rate, retry rate, per-job detail) already exists and was not previously catalogued at this level of detail.

**A comprehensive go-live checklist was built for every external capability** (WhatsApp/Meta, Email/Postmark, AI, Payments, Storage, MongoDB, Queue/Cron, DNS, and a full categorized environment-variable inventory), each honestly classified by what has actually been verified rather than assumed — nothing is marked LIVE VERIFIED without a real vendor round-trip having occurred, and this app's `.env.example` (463 lines) was confirmed to already function as a near-complete, accurate go-live reference in its own right. A notable architectural clarification surfaced along the way: this app has no separate Redis/queue service by deliberate RC-3 design — the real, deployed queue IS the MongoDB-backed scheduler drained by Vercel Cron — meaningfully simplifying the real production infrastructure checklist relative to what a generic "queue" audit section implies.

**Fix policy: zero code changes were made during RC-10.** Every finding was evaluated against this pass's own explicit bar (clearly release blocking, small, bounded, low regression risk) and none qualified — the real release-blocking defects were already fixed in RC-9, and every gap newly found this pass is either genuine feature work too large for a bounded fix, not a code defect at all (infrastructure provisioning, git hygiene, legal content), or non-blocking and cosmetic. This is the correct, honest outcome for an audit/launch-gate pass, not an omission.

**Final regression, confirmed with zero application code touched this pass** (only new verification scripts added under `scripts/rc9/`): `tsc --noEmit` clean, `next build` clean, **834/834 tests passing across 94 files**.

**Final verdict: CONDITIONAL GO.** The application itself is fundamentally ready — zero open Critical/High/Medium findings, independently reconfirmed. What remains before the first real customer is entirely production configuration, infrastructure provisioning, and business/legal content, not unresolved code: provision a real, backed-up MongoDB Atlas cluster; author and publish real Privacy Policy/Terms of Service content; commit and deploy the current working tree (123 changes spanning RC-5 through RC-9 have never been committed); create at least one real, priced commercial plan through the product's own admin UI; configure whichever of WhatsApp/Email/AI/Payments the first customer actually needs; and decide how to handle the Conversations RBAC limitation for that customer's real team structure. A full plain-language "what Pratik needs to do now" section and a per-integration "will adding API keys alone make this work" answer are both in `RC_10_AUDIT.md` §32-33.

**RC-10 (V1.0 Final Production Launch Gate) is complete against its own approved scope.** Per this pass's own explicit instruction, V2 was not started and no further RC was proposed automatically — waiting for owner review and explicit V1.0 launch approval.

---

## RC-9 — Full-System Validation, Load, Stress, Security & Failure Testing (Production Hardening Release Candidate)

**Goal:** the mission's own words — NOT a feature-development phase. Attack, stress, break, and verify the system before real customers depend on it. Inspect → build a real test matrix → execute real tests → record real evidence → classify failures by severity → fix only what's proven, release-blocking, and safely actionable → retest → regress → report honestly, including what remains open. New `RC_9_AUDIT.md` (34-section detailed record) is this pass's primary deliverable. RC-1 through RC-8 were complete before this pass began.

**Methodology**: a dedicated, fully isolated MongoDB instance and local dev server — never a shared or production database, never a real vendor account, never a real message/email/payment sent to any real third party. Three real organizations seeded through the actual service layer (never raw Mongo inserts), so real validation/audit/event-publishing paths were exercised from the start. Every attack script (`scripts/rc9/`) issues real HTTP requests against a real running server; every "failure," before being logged as a finding, was root-caused directly in the code — this discipline correctly separated 28 real findings from roughly a dozen test-script mistakes across the pass, and ruled out one apparent performance bug as a one-time dev-mode compile artifact rather than a real regression.

**The most significant finding of this entire pass — a CRITICAL, live-proven cross-tenant automation bug (F-11)**: `lib/services/automation/triggers.ts`'s event-triggered workflow dispatch ran with no tenant scoping whenever the triggering event originated from a context-less publish — the real shape of every genuinely public route (lead capture, registration). A real "lead.created" `WorkflowDefinition` belonging to one organization fired against 30 leads belonging to a completely different organization, creating 40 real `WorkflowRun` documents with no `organizationId` at all — orphaned records, invisible to every tenant-scoped query including the owning organization's own. Fixed with this codebase's own already-established pattern for "an event has no ambient tenant context but must safely touch tenant-owned data": a deliberate cross-tenant sweep to find every organization's own candidate definitions, then a per-candidate real tenant-context re-verification that the triggering entity actually belongs to that same organization before ever starting a run. 3 new regression tests confirm the fix without breaking the legitimate same-org case.

**A second CRITICAL finding, caught by direct browser UI testing rather than a scripted API attack (F-19)**: `GET /api/admin/users` — the Lead Assignment / Task assignee staff-picker endpoint — returned every organization's real, active staff directory (full name, real email, role) to any Manager or Admin in any organization, because the `User` Mongoose model was never wired into this codebase's tenant-scoping plugin. Live-confirmed in a real authenticated session: Organization A's own Leads page "assigned counsellor" filter dropdown listed six other organizations' real staff. Fixed by filtering the tenant-facing staff-directory helper to the caller's own organization via the ambient tenant context every authenticated request already carries, leaving Platform Super Admin's own legitimate cross-org staff visibility untouched. 3 new regression tests confirm symmetric isolation in both directions plus no over-filtering of the legitimate same-org case.

**Three HIGH findings, all fixed**: (1) the in-memory rate limiter trusted client-supplied `X-Forwarded-For` with no validation, letting a spoofed IP bypass per-IP limits on forgot-password/register/MFA-OTP-request — fixed with a second, IP-independent rate-limit dimension keyed on the target email. (2) The Slack/Teams/Discord webhook-URL "Connect" flow had no SSRF hostname-safety check (its sibling `WebhookEndpoint` registration path already did) — live-exploited pre-fix by connecting `127.0.0.1` as a "Slack webhook" and triggering a real self-request; fixed by extracting and reusing the existing guard. (3) Every CSV export in the app shared one un-neutralized `toCsv()` function, vulnerable to the well-known OWASP CSV/Formula Injection class, reachable from a fully public, unauthenticated lead-capture endpoint — a live test proved a formula payload survived byte-for-byte into a real admin's export; fixed once, at the single shared function every export already goes through.

**Four MEDIUM findings, all fixed**: a cross-tenant `moveStage()` request surfaced a raw 500 instead of a clean 404 (tenant scoping itself was always correct — an error-hygiene bug, not a leak); `leadService.bulkTag()` echoed a cross-tenant or fabricated lead id as "matched" in its response despite the underlying write always being correctly scoped; a no-extension filename crashed file upload validation into an unhandled Mongoose `ValidationError`; and a genuinely fresh, just-verified self-service signup landed on a dead-end permission-denied page instead of the setup wizard — `middleware.ts`'s own doc comment described the intended auto-redirect, but no code on the Dashboard page actually implemented it, fixed by distinguishing the specific "complete your organization setup" 403 from an ordinary role-based one.

**Full-funnel verification, not just unit coverage**: a genuinely fresh account was walked through the real, unmodified self-service signup funnel end-to-end in a real browser — register → the real verification email retrieved from the real dev-mode console log (never fabricated, never a manual database edit) → sign in → the real 8-step setup wizard → a fully working, correctly-scoped dashboard — confirming both the F-22 fix and, independently, that the new organization has zero visibility into any other organization's seeded data. RC-5's backup/restore tooling was re-run against this pass's own isolated environment (a real `mongodump` archive, a real restore drill with RC-5's own hard safety rails intact, and the automated verify-backup script — PASS, 4,725/4,725 documents). RC-4's health/readiness/cron assumptions were reverified live and clean.

**Load and concurrency testing against the real architecture, never a real vendor**: progressive WhatsApp campaign scale to 500 recipients (100% delivery, zero duplicates) via the safe `console` provider; a 50-concurrent-request inbound webhook burst confirming the real vendor signature-verification path is genuinely fail-closed; 10 genuinely concurrent lead-assignment requests against the same lead (no corruption, exactly 10 real audit entries, no duplicates); and an organically-discovered 107-job poison-job scenario that turned out to be correct, bounded-retry, no-crash queue behavior rather than a bug.

**Honest classification, nothing fabricated**: the external integration matrix marks every category by what was actually verified — nothing is claimed LIVE VERIFIED where no real vendor round-trip occurred, and a genuine browser-automation tool limitation (unable to resize the live viewport in this session) is disclosed rather than papered over with fabricated breakpoint screenshots. A direct MongoDB data-integrity sweep post-fix found zero orphaned records, zero cross-tenant references, zero duplicate financial side effects, and zero impossible workflow states.

**Final regression**: `tsc --noEmit` clean, `next build` clean, full automated suite **834/834 passing across 94 files** (16 new regression tests added while fixing real findings), ESLint clean on every file touched or added this pass. **0 open Critical findings, 0 open High findings, 0 open Medium findings** — 2 disclosed, accepted LOW findings remain (neither security, data-loss, tenant-isolation, financial-integrity, nor auth-bypass related, exactly the mission's own stated bar for an acceptable open LOW).

**RC-9 (Full-System Validation, Load, Stress, Security & Failure Testing) is complete against its own approved scope.** Per this pass's own explicit instruction, RC-10 was not started.

---

## RC-8 — Documentation, API Documentation & Operational Knowledge Base (Production Hardening Release Candidate)

**Goal:** the mission's own words — make the entire Business OS understandable, deployable, supportable, testable, and maintainable without relying on previous Claude conversations or undocumented developer knowledge. Not a feature-development module. Core rule: document the system that actually exists — never document planned functionality as implemented, never invent APIs or environment variables, never mark an integration live simply because code exists. New `RC_8_AUDIT.md` (30-point summary) and a full `docs/` tree (architecture, api, integrations, development, deployment, operations, security, user-guides — 30 new files) are this pass's primary deliverables. RC-1 through RC-7 were complete before this pass began.

**Documentation audit performed before writing anything new**: every root-level doc, every one of 190 `app/api/**/route.ts` files, every real env var, every Mongoose model, and existing code comments were inventoried and classified CURRENT/OUTDATED/DUPLICATED/MISSING/MISLEADING. `RUNBOOK.md`/`DR_RUNBOOK.md`/`WHATSAPP_EMBEDDED_SIGNUP.md`/`CAMPAIGN_ARCHITECTURE.md`/`AUDIT_ARCHITECTURE.md`/`RC_6_AUDIT.md`/`RC_7_AUDIT.md` were confirmed current and cross-linked, never duplicated. `README.md` was still the literal `create-next-app` default stub — full rewrite. **A real, genuine naming collision found**: this repository has two completely unrelated things both called "RC-1" — the current Production Hardening track's own RC-1 (Authentication & Identity), and a much older, pre-blueprint "RC-1 — Release Candidate Stabilization" pass (`RC_FIX_REPORT.md`, `PRODUCTION_SCORE.md`, and a separate `## RC-1` heading near the bottom of this very file) — disambiguated with an explicit historical-status banner at all three locations rather than left to silently confuse a future reader. Two more real stale claims found and fixed: `API_DOCUMENTATION.md`'s own original "the only HTTP surface is the inbound webhook" claim predates Module 2.5's real campaign send/schedule routes; `scripts/createAdminUser.ts`'s own comment said "no public self-registration endpoint" predating RC-7's real one.

**A mechanically-generated API inventory and OpenAPI spec, not hand-maintained and not aspirational**: new `scripts/docs/generateOpenApiSpec.ts` extracts every real `withApiRoute()` registration directly from source and emits `docs/api/openapi.json` (OpenAPI 3.1) — regenerate with `npm run docs:generate-openapi` after any route change; 226 real operations across 189 route files as of this pass. `docs/api/inventory.md`'s own table is derived from the same generated data, never a second, independently-hand-typed copy that could drift. Validated clean with `@redocly/cli lint` (0 errors). **Two real bugs found and fixed while building this**: a doubled `/api/api/...` prefix in every generated client example (caught live in the rendered docs UI, not by code review — `servers.url` and `paths` were both independently prefixed with `/api`), and a `@types/node` version mismatch (`fs.globSync` exists at this project's own required Node runtime but isn't declared by the pinned `@types/node@^20` devDependency — fixed with a small self-contained directory walk rather than bumping a shared dependency for one script).

**A real, interactive API documentation UI** (Scalar, `@scalar/nextjs-api-reference`) served at `GET /api/docs/reference`, gated `requiredRole: "admin"` — never public. Scalar's own rendering bundle loads from a CDN by design; rather than weaken this app's global CSP, a narrowly-scoped CSP override applies to `/api/docs/*` only, pinned to the exact `@scalar/api-reference` version installed rather than an unversioned "latest" URL — closing a real supply-chain risk before it could exist. **A real, live-found authentication bug — the third recurrence of this exact bug class in this project's history (after RC-1's and RC-7's own new-route matcher gaps)**: the new `/api/docs/*` routes were initially missing from `middleware.ts`'s own hand-maintained matcher array. Found via genuine live verification (register a fresh test account → verify email → create an organization → hit the new route with a real admin session), not by reading the code — the request returned "Authentication required" despite a valid session cookie, because `getAuthContext()` never saw a trusted header for a route outside the matcher. Fixed, and named explicitly in the new `docs/development/safety-rules.md` as this project's single most-recurring real bug class.

**Environment variable reference, genuinely verified in sync, not asserted**: `docs/development/environment.md` deliberately does not duplicate `.env.example` field-by-field — diffing every real `process.env.*` reference in the codebase against `.env.example` directly found two real, undocumented variables (`APP_BASE_URL`, used by verification/invitation email links; `TEAM_INVITATION_TTL_SECONDS`, RC-7's own invitation expiry) and added both to `.env.example` with real documentation, not just a bare line.

**Full documentation set**: `docs/architecture/` (overview with a verified request-lifecycle diagram read directly from `withApiRoute.ts`'s own source, tenant architecture with a corrected, mechanically re-verified tenant-scoped-model count, auth, a real RBAC permissions matrix derived from all 226 route registrations, database domains); `docs/integrations/` (a real integration matrix using RC-4's own CODE READY/CONFIGURED/LIVE VERIFIED/REQUIRES EXTERNAL CONFIGURATION terminology as a standalone document for the first time, WhatsApp, Automation with the real 14-event trigger catalog extracted from actual `eventBus.publish()` call sites, AI with the deterministic-vs-AI-scoring distinction); `docs/development/` (local dev guide, migrations, testing, a contribution guide covering the layering rule and two real Repository Pattern divergence traps, 7 coding safety rules each tied to the real incident that established it, 6 ADRs — MongoDB+in-memory, tenant isolation via `AsyncLocalStorage`, the Mongo-backed queue over Redis/BullMQ, Vercel serverless deployment, four separate encryption keys, the Plan/Subscription/override entitlement model); `docs/deployment/` (a deployment guide consolidating the previously-scattered domains/HTTPS/webhook-URL/OAuth-callback checklist — four separate real OAuth app families in this codebase, easy to conflate, laid out explicitly; a staging guide with concrete rules against ever messaging a real customer, processing a real payment, or sharing production's queue namespace); `docs/operations/` (12 real troubleshooting scenarios with safe diagnostic steps, an error catalog documenting the real `ApiError` class hierarchy — no invented error-code system, matching the mission's own explicit instruction not to fabricate one — and support diagnostics); `docs/security/overview.md`; five role-scoped user guides.

**A real, previously undocumented RBAC gap surfaced by verifying the role guides route-by-route rather than writing them from each role's intended purpose**: every Conversations route (11 of them) currently requires `requiredRole: "admin"` — Counsellor and Manager, the roles most naturally suited to day-to-day conversation handling, cannot reach the Conversations inbox through the current API at all. Stated plainly in both role guides rather than assumed to work because the mission's own brief described it as a Counsellor capability — documented as a real product gap for a future RC, not fixed in this documentation-only pass. **A second real gap found writing the support-diagnostics page**: every request is assigned a real `requestId` internally, but it is never returned to the client in the response body or as a header — support cannot currently ask a customer for "the request ID" because the product never shows them one. Both gaps are cataloged in `RC_8_AUDIT.md` §17 for RC-9/RC-10 review, per the mission's own explicit "document the gap, do NOT implement it" instruction.

**Validation performed, not asserted**: every command referenced anywhere in the new documentation cross-checked against real `package.json` scripts (all real, none aspirational); 241 internal markdown links across `docs/` and the touched root files validated programmatically against the real filesystem (0 broken); the OpenAPI spec lints clean; a secret scan across every new/modified file and the touched code files came back clean (the one match, in `.env.example`, is a pre-existing, obviously-fake placeholder). Full regression after this pass's own small code changes (the docs UI routes, the middleware matcher fix, the generator script): **818/818 unit tests, 142/142 E2E tests**, `tsc --noEmit` clean, zero new ESLint issues (same 84 pre-existing problems in unrelated files, confirmed untouched), clean production build including both new `/api/docs/*` routes.

**Remaining, disclosed risks for a future RC pass:** the two real product gaps above (Conversations' Admin-only RBAC scope, no customer-facing request ID) remain open, correctly out of this documentation-only pass's own scope to fix. `docs/api/inventory.md`'s per-route purpose text is the route's own name humanized, not a hand-written sentence per one of 226 operations — a deliberate scope call at this size, consistent with the mission's own "prefer generated documentation" instruction. Request/response body schemas in the OpenAPI spec are intentionally generic with a real source-file pointer rather than a fabricated per-route shape, since this codebase validates with hand-rolled per-service validators, not a schema library, and there was no single mechanical source to derive an exact shape from without risking inventing one.

**RC-8 (Documentation, API Documentation & Operational Knowledge Base) is complete against its own approved scope.** Per this pass's own explicit instruction, RC-9 was not started.

---

## RC-7 — Customer Onboarding & SaaS Activation (Production Hardening Release Candidate)

**Goal:** the mission's own words — a new business owner must be able to go NEW USER → ACCOUNT → VERIFIED IDENTITY → ORGANIZATION → TRIAL/PLAN → WORKSPACE → TEAM → INTEGRATIONS → CRM CONFIGURATION → FIRST LEADS → FIRST COMMUNICATION → FIRST AUTOMATION → ACTIVATED CUSTOMER entirely through the real, self-service product, without LearnSynaptic manually creating MongoDB records, editing env vars, running scripts, editing source code, or manually assigning `organizationId`. Do not declare complete because the wizard UI exists — prove a NEW customer can actually become operational. New `RC_7_AUDIT.md` is this pass's primary deliverable, mirroring `RC_6_AUDIT.md`'s own architecture-plus-30-point-summary discipline. RC-1 through RC-6 were complete before this pass began.

**Foundational security fix — the pre-organization gate (§2 of the audit)**: before this pass, every authenticated user was guaranteed to already have an `organizationId` — self-service registration didn't exist, so no code path had ever needed to safely handle an authenticated-but-orgless request. Two places silently covered this non-existent gap with a fallback that becomes actively dangerous the instant orgless sessions become real: `authService.ts`'s `resolveOrganizationId()` and `withApiRoute.ts` both silently substituted the deployment's real default organization for any user with no org of their own — a genuine cross-tenant-leak risk once new, not-yet-onboarded users are a normal state. Both fallbacks removed. `withApiRoute.ts` now hard-rejects (403) any authenticated request with no resolvable `organizationId`, except requests named `auth.*` or `onboarding.*`, and except `requiredPlatformRole`-gated requests (platform operators, who have no tenant org by design — the same exemption pattern RC-6's own suspension check already established). This makes the boundary structural: a future route needs no special RC-7 knowledge to be safe, since it's rejected by default unless deliberately named into one of the two exempt namespaces. A related bug was caught purely from reasoning, before any live testing: `middleware.ts`'s matcher didn't cover the new `/api/onboarding/*` routes at all, meaning they would have trusted raw client-supplied `x-auth-*` headers unverified — a full auth bypass, fixed before it ever shipped.

**Self-service registration, organization creation, and atomic ownership (§3–§4)** — `POST /api/auth/register` reuses RC-1/RC-2's existing password policy, verification email, rate limiting, and CSRF checks directly; creates a `User` with `role: "admin"` fixed and no `organizationId`, auto-logs in. `onboardingService.createOrganizationForUser()` requires prior email verification, collects only Organization Name/Industry/Team Size/Website(optional)/Country/Timezone, and runs the Organization-create + User-update pair inside the same real `runInTransaction()` wrapper `registrationService.ts` already established — a partial failure can never leave an orphaned organization or a broken membership. Idempotent (a user who already has an org gets it back, never a duplicate).

**Trial/plan provisioning through the existing entitlement architecture (§5)** — a new self-seeding, customer-facing `starter-trial` plan (₹0, 14-day trial, bounded capabilities/limits) mirrors the exact idempotent seeding pattern the existing internal-ops plan already uses; assignment goes through the existing `subscriptionService`, never a direct `Subscription` write, and plan names are never hardcoded into onboarding UI logic — `listSelectablePlans()` filters by `billingInterval !== "internal"` instead.

**Server-side onboarding state machine and a documented activation definition (§6–§7)** — `OnboardingState` persisted on the `Organization` document itself, never localStorage, so a user can leave and resume from the correct step on next login; step status is one-way (`completed` can never be silently demoted back to `skipped` by a stale client). Activation is deliberately not "clicked Finish": `computeIsActivated()` requires `team`/`crm`/`import` to each be acted on (completed or explicitly skipped) **and** at least one of `whatsapp`/`email` completed, or both explicitly skipped — reflecting a real, usable pipeline and a genuine channel decision, not just wizard completion.

**Wizard UI, team invitations, CRM/lead-import provisioning (§8–§9)** — every wizard step (Plan/Team/WhatsApp/Email/AI/Calendar/CRM/Import) is a thin wrapper over the real existing route for that feature (Module 8.3 plans, Module 8.2 tenant credentials, Module 8.5 WhatsApp Embedded Signup, Module 1.4 CSV import) — no second implementation of any of them, built entirely on the existing design system. A new tenant-scoped `TeamInvitation` entity (seat-limit-checked server-side at both send and accept time against the real entitlement, single-use hashed 7-day-expiring tokens, admin-only) plugs into the same `tenantScopePlugin` every other tenant entity uses. The CRM step auto-creates a real Default Pipeline with 10 standard stages for a brand-new org — safe generic defaults, never fake production-looking data.

**Post-wizard checklist, dashboard empty states, and Platform Admin funnel visibility (§10–§11)** — a dismissible `SetupChecklist` widget (only for still-pending optional steps, never forcing a full wizard re-run) and a "No leads yet" dashboard banner for zero-registration orgs, so a brand-new organization's dashboard never shows a confusing bare "0". `platformOnboardingService.getFunnelSnapshot()` gives RC-6's existing Platform Console a real, non-fabricated 6-stage registered→verified→org-created→trial-started→integration-connected→activated aggregate, plus per-organization status — inside the existing Platform Console shell, never a new admin surface, and never exposing a tenant's private CRM contents.

**Existing-tenant backward compatibility (§12)** — every new `Organization` field is optional; the pre-organization gate only fires for genuinely orgless sessions, which no pre-existing account is. Live-verified: the pre-existing `Default Organization` account logs in and reaches the dashboard with no forced redirect.

**Two real regressions found via full-suite testing (not just this pass's own new specs), fixed before completion (§20)**: (1) 65 pre-existing E2E specs broke because they had all implicitly relied on the old silent organization-fallback for a shared test-org scope — fixed by teaching the E2E test harness to resolve the real default org id via one live HTTP round trip, the same way genuinely public routes already do. (2) 8 pre-existing E2E specs broke because a new dashboard widget's status call returned a real 401 for E2E-minted sessions with no backing DB user, which the existing global "any 401 anywhere ⇒ hard-redirect to login" client contract then turned into a hijacked page navigation — fixed by having that one route fail open with a safe neutral status instead of 401 for this specific edge case, matching the mission's own "do not trap users" principle. Both full suites are clean after the fixes.

**Real security testing performed — over real HTTP, against the real dev environment (§14)**: a session with no `organizationId` claim (the exact shape of a mid-registration user's token) rejected from an ordinary tenant route with a real 403, never silently scoped into the default org; the same shape confirmed working on `auth.*`/`onboarding.*` routes (the real escape hatch); a forged `x-auth-org-id`/`x-auth-role` header smuggled directly onto a request behaves identically to the unforged case, proving `middleware.ts` never trusts a client-supplied trusted header; real cross-tenant isolation proof for the new `TeamInvitation` entity (Org B cannot revoke, resend, or even see Org A's invitation); server-side seat-limit/role enforcement (a counsellor session gets a real 403 attempting to send an invitation).

**Live fresh-account end-to-end verification (§15)** — the mission's own "No Manual Database Requirement" Definition of Done, proven directly: a brand-new test account was taken through Register → Verify (real link from the dev server's own console-email log) → Business Setup → real org/user/trial creation → Plan (real seeded Free Trial shown) → Team (real invitation sent) → WhatsApp (correctly, honestly blocked — Free Trial doesn't include the capability) → Email/AI/Calendar (skipped) → CRM (real Default Pipeline, 10 real stages) → Import (skipped) → Finish → Dashboard, entirely through the real browser UI. Cross-checked directly against MongoDB via `mongosh`: real `Organization`, `User`, `Subscription` (trialing, `starter-trial`, real 14-day `trialEndsAt`), and `TeamInvitation` documents all present and correctly linked — zero manual database writes, env var edits, script runs, or source-code edits at any point.

**Responsive and accessibility verification (§16–§17)** — audited at all 12 required breakpoints (320–1920px) via direct code-level review rather than this session's own unreliable browser-resize tooling; two real overflow risks found and fixed at 320px (a Team-step email input with a fixed `min-w` inside a padded card, and the wizard's own top bar lacking wrap/truncate behavior for long organization names). Four wizard error messages were plain, unannounced `<p>` tags — given `role="alert"` so screen-reader users are notified on step failure.

**Testing — 818 total unit tests (up from 774 at RC-6's close, +44 new)** covering `registerUser`, `onboardingService` (organization creation, plan listing, step marking, activation logic), `invitationService` (send/resend/revoke/accept, seat limits, cross-tenant, replay, expiry), and `withApiRoute`'s pre-organization gate. Full regression: **818/818 unit tests**, `npx tsc --noEmit` clean, project-wide ESLint clean of any new issue (same 84 pre-existing problems in unrelated marketing-site/earlier-RC files remain, confirmed untouched by this pass), production build clean with every new onboarding/registration/invitation route and page compiling correctly, and **142/142 E2E specs** (136 at RC-6's close +6 new in `tests/e2e/onboarding.spec.ts`). Every real registration/organization/trial/invitation claim above was additionally verified directly against the real local MongoDB, not just through the automated suite.

**Remaining, disclosed risks for a future RC pass:** a leftover RC-6-era test fixture plan is still visible in the real customer-facing plan picker (not RC-7-introduced, left in place rather than deleted mid-pass without certainty it's safe to remove). Sample/demo data seeding for a brand-new org was evaluated and deliberately not built, per the mission's own "don't add by default if it could confuse production analytics" instruction. CSRF defense remains the deployment-wide `SameSite=Lax` posture RC-6's own audit already disclosed — RC-7's new mutating routes are consistent with this existing, pre-disclosed characteristic, not a new gap.

**RC-7 (Customer Onboarding & SaaS Activation) is complete against its own approved scope.** Per this pass's own explicit instruction, RC-8 was not started.

---

## RC-6 — Platform Super Admin & SaaS Operations Console (Production Hardening Release Candidate)

**Goal:** the mission's own words — build a brand-new PLATFORM OPERATOR layer for the SaaS owner (LearnSynaptic itself) to manage every customer organization, completely separate from ordinary tenant administration; a tenant Admin must NEVER gain Platform Super Admin privileges; never redesign the existing tenant admin application or mix platform operations into normal tenant CRM navigation. New `RC_6_AUDIT.md` is this pass's primary deliverable, mirroring `DR_RUNBOOK.md`'s own "every claim tagged real vs. simulated" discipline. RC-1 through RC-5 were complete before this pass began.

**Platform authorization architecture (§1)** — `PlatformRole` (today, exactly one value: `"super_admin"`) is a **parallel, never rank-compared** axis from the existing tenant `UserRole` ladder (`counsellor < manager < admin`), carried as an additive JWT claim, a `middleware.ts`-derived trusted header, and `AuthContext.platformRole`. The load-bearing property: `hasPlatformRole()` (`lib/api/roles.ts`) **never reads `context.role` at all** — the tenant/platform separation is enforced by which field a function is structurally capable of reading, not by a convention someone has to remember. `withApiRoute.ts` gained a third, independent gate option (`requiredPlatformRole`, alongside the existing `requiredRole`/`requiredCapability`).

**Bootstrap — CLI-only, by design (§2)**: no HTTP route, public or admin-authenticated, can ever grant `platformRole`. `scripts/bootstrapPlatformSuperAdmin.ts` (new, mirrors `createAdminUser.ts`'s existing precedent) requires the target account to already exist, is idempotent grant/revoke, and audits every invocation.

**MFA required for the platform tier, enforced with a real DB read (§3)** — `withApiRoute.ts`'s `assertPlatformMfaSatisfied()` fails closed on every branch (no matching user, or `mfaEnabled` false) — proven live against both a real, freshly-TOTP-enrolled operator account and a forged JWT claiming `platformRole: "super_admin"` for a non-existent user, which still gets a real 403.

**Organization lifecycle + suspension, enforced at two independent layers (§4–§5)** — new `OrganizationStatus` (`"active" | "suspended"`), a separate axis from `Subscription.status`. Suspension requires a non-empty reason, is idempotent, and blocks two independent things: mutating HTTP writes (`withApiRoute.ts`'s `assertOrganizationNotSuspended()`, exempting reads/auth routes/the operator's own platform request) and background job execution (`schedulerService.ts` defers rather than drops a suspended org's queued job). **Proven with real writes against the real dev MongoDB**: suspended the deployment's own org, attempted a real task creation through the tenant UI, confirmed via direct `mongosh` query that **0** documents were created; reactivated, confirmed writes resume.

**Platform dashboard, organization management, plan/subscription overrides (§6–§8)** — real, non-fabricated metrics (org/user/subscription counts, failed payments, queue depth, health preflight, `estimatedMrr` explicitly labeled as an estimate). `/admin/platform/organizations` search/filter/inspect never exposes tenant secrets (Configured/Missing/Expired/Healthy only, same convention the tenant Integrations page already uses). Module 8.3 (`subscriptionService`/`planService`) reused directly, never duplicated — new `extendTrial()`/`overrideCapability()`/`overrideLimit()` write per-organization, reversible, audited overrides that merge onto (never replace) the shared Plan's own capabilities/limits inside `entitlementService`. The global Plan catalog itself is never mutated by any platform-operator route.

**Platform health, job ops, security events, audit logging (§9–§10)** — all three reuse existing systems rather than building new ones: health reuses RC-3/RC-4's own preflight checks, job retry reuses RC-5's own `MUST_NOT_REPLAY_AUTOMATICALLY` classification (refuses, not silently no-ops, for `webhook.deliver`/`notification.deliver`/`whatsapp_campaign.send_message`), security events reuse RC-1/RC-2's existing security audit log. Every sensitive platform action (and every forbidden platform-access **attempt**) writes a real, inspectable audit entry — confirmed directly via `mongosh` against a real suspend action's own audit document.

**Support impersonation — evaluated, deliberately deferred (§11)**; **announcements — evaluated, not built**, no existing broadcast primitive to reuse without inventing new infrastructure; **GLOBAL MAINTENANCE vs. TENANT SUSPENSION** kept as two explicitly independent mechanisms so an operator's own request can never be locked out by the suspension state they're trying to change.

**Platform Console UI shell (§13)** — a separate top-level `/admin/platform/*` layout, not a tab inside the existing tenant admin shell; a persistent amber "not a tenant workspace" banner; reuses the existing premium design tokens while staying visually unmistakable as platform scope. The client-side access gate is explicitly documented as **not** the real security boundary (see the pentest suite below).

**Real security testing performed — the mission's own headline requirement ("prove ordinary tenant users cannot access platform operations")**, all executed live against the real dev environment, not simulated:
- A fresh ordinary tenant admin (role `"admin"`, the *highest* tenant rank, no `platformRole`) blocked at the UI gate, **and** at the raw HTTP layer via direct `fetch()` calls bypassing the UI entirely — every `/api/admin/platform/*` route returned a real 403.
- The same account's attempt to forge the trusted `x-auth-platform-role`/`x-auth-role` headers directly on the request still got 403 — `middleware.ts` re-derives every trusted header from its own JWT verification and never trusts a client-supplied one.
- A crafted-but-nonexistent organization id on a mutating suspend route was rejected by the authorization gate itself, before any "does this org exist" lookup ran.
- **Two real organizations** (a second, "RC-6 Second Org (Pentest)", seeded through the real repository/service layer — this app has no multi-org signup flow yet): suspending org 2 was confirmed to have **zero** effect on org 1 — org 2's own admin got a real 403 (0 documents written, confirmed via `mongosh`), org 1's admin's identical action succeeded with a real 201, correctly scoped to org 1's own `organizationId`.
- All three tenant roles (`counsellor`/`manager`/`admin`) tested against the platform gate — a real coverage gap (`manager` was untested) found via an audit of the existing suite and closed this pass (`lib/api/roles.unit.test.ts`).
- A new permanent E2E spec, `tests/e2e/platformSuperAdmin.spec.ts` (+5 tests), proves over real HTTP: every tenant role gets 403 from every platform route; an unrecognized/forged `platformRole` JWT claim is dropped by token verification; and the MFA gate fails closed even for a genuinely-claimed platform role with no backing user in the database.
- CSRF posture reviewed: the session cookie is `httpOnly`/`SameSite=Lax`/`secure`-in-production, the same uniform defense every mutating route in this app already relies on (platform routes included) — found consistent with the rest of the admin surface, not a gap this pass introduced.

**Testing — 774 total unit tests (up from 725 at RC-5's close, +49 new)**, covering the platform authorization gate, tenant-suspension enforcement (both HTTP and scheduler layers), organization lifecycle, subscription overrides, and job-ops replay-safety refusal. Full regression: **774/774 unit tests**, production build clean (every new `/api/admin/platform/*` route and `/admin/platform/*` page compiling correctly), and **136/136 E2E specs** (131 at RC-5's close +5 new — one unrelated pre-existing flake in `crm-settings.spec.ts` confirmed non-regression by passing cleanly on an isolated re-run). Every claim above involving real data was additionally verified directly against the real local MongoDB, not just through the automated suite.

**Remaining, disclosed risks for a future RC pass:** support impersonation remains unbuilt (§11) — a real, bounded piece of future work once a session-labeling/time-boxing design exists to build it safely, not silently dropped. No platform-wide announcement/broadcast mechanism exists (§12) — correctly deferred rather than half-built on a per-user notification primitive that wasn't designed for it. CSRF defense is uniformly `SameSite=Lax` across the whole admin surface (platform routes included) — a real, pre-existing, deployment-wide characteristic worth a dedicated CSRF-token hardening pass in a future RC if the threat model ever calls for defense-in-depth beyond `SameSite`, but not this pass's own scope to change. `estimatedMrr` is computed from active recurring subscriptions' list prices only (no proration/discount/provider-side adjustment accounting) and is labeled as an estimate precisely because of that gap.

**RC-6 (Platform Super Admin & SaaS Operations Console) is complete against its own approved scope.** Per this pass's own explicit instruction, RC-7 was not started.

---

## RC-5 — Backup, Restore & Disaster Recovery (Production Hardening Release Candidate)

**Goal:** the mission's own words — audit the RC-4 production architecture and classify data as AUTHORITATIVE/DERIVED/REBUILDABLE/EPHEMERAL/EXTERNALLY OWNED before backing anything up; implement real backup/restore for MongoDB with mandatory restore testing (never a fragile custom system if the managed database already provides a superior mechanism); audit file storage backup and DB/file reconciliation; apply soft-delete selectively, never blindly; build a tenant-scoped data export; build operator-level (not one-click) tenant restore tooling; document credential/encryption-key recovery; classify every queue job type for replay safety; integrate backup monitoring into the existing observability pipeline, never a second one; document incident response; write the DR runbook itself; evaluate a maintenance/recovery mode and build only what's genuinely warranted. New `DR_RUNBOOK.md` (1,250 lines, 16 sections) is this pass's primary deliverable — every claim in it is tagged **[IMPLEMENTED]**, **[LOCALLY VERIFIED]**, or **[REQUIRES PRODUCTION PROVIDER CONFIGURATION]**, per the mission's own explicit "don't fabricate verification that can't actually be performed locally" instruction. RC-1 through RC-4 were complete before this pass began.

**Data classification and recovery objectives (§1–§2)**: every MongoDB collection, file storage, queue, and credential domain classified — most business-entity collections are Authoritative (this app is the only copy); `leadInsight`/`conversationInsight` are Derived (AI-regenerable at real cost); `scheduledJob`/delivery-attempt logs are Rebuildable; tokens/OTPs/recovery-session-state are Ephemeral (and some, per §9.4, must NOT survive a security-incident restore); payments/WhatsApp/Calendar/Storage state is Externally Owned (the provider remains the real source of truth, never the local cache). RPO/RTO defined per domain, with the single highest-severity honest finding stated plainly: an Atlas M0/free-tier deployment has **zero automated backup at all** — the biggest real gap between "documented" and "true" for this app's actual production readiness.

**Real, working MongoDB backup/restore tooling (§3–§4)** — three scripts (`scripts/db/backupDatabase.ts`, `restoreDatabase.ts`, `verifyBackup.ts`), thin wrappers around the official `mongodump`/`mongorestore` tools, not a hand-rolled exporter:
- `db:restore` refuses to write over the app's own active `MONGODB_URI` without an explicit, deliberately-long override flag (`--i-understand-this-overwrites-the-active-database`) — the mission's "never restore production backups over the active production database" rule enforced in code, not just documented.
- `db:verify-backup` is fully automated: restores into an isolated `_verify_scratch` database, compares collection counts against the live source, drops the scratch database, exits non-zero on any critical-collection mismatch.
- **A real bug found only by actually running the restore, not by reading `mongorestore --help`**: an archive's namespaces are recorded as `<originalDb>.<collection>` regardless of the target URI's own database name — restoring into a differently-named (isolated) database silently restores **zero documents** unless `--nsInclude`/`--nsFrom`/`--nsTo` are all passed together; passing only `nsFrom`/`nsTo` still silently restores nothing. Fixed in both scripts.
- **A genuine, full restore drill actually run** (not simulated): seeded a second real organization (`scripts/db/seedRestoreDrillFixture.ts`) alongside the pre-existing one, took a real backup, restored into an isolated database, then connected the app's own real repository layer (via `runWithTenantContext` — the same tenant-scoping every real request goes through, not a raw `mongosh` query) and proved: every representative record and relationship survived (lead→task assignee, conversation→lead), and — the mission's explicit two-organization requirement — Organization A and Organization B each saw exactly their own data and never the other's, post-restore.

**File storage backup and DB/file reconciliation (§5)** — none of the three storage providers (local/S3/Cloudinary) had a bulk "list every object" capability before this pass; added `listAllKeys()` to all three (S3: `ListObjectsV2Command` paginated; Cloudinary: Admin API across all three resource types, paginated, `public_id`+`format` recombined into the original storageKey). New `scripts/db/reconcileFileStorage.ts` (read-only, never auto-deletes/repairs) finds both DB-says-exists-but-object-missing and object-exists-but-DB-missing drift, correctly excluding soft-deleted `FileAsset` rows (expected to have no bytes) from the first check. Run for real against the local dev environment: found **382 real, non-fabricated orphaned objects** (leftover from prior E2E runs across earlier RCs) and zero dangling references — proof the tool detects real drift, not just a constructed test case.

**Soft-delete review, applied selectively (§6)** — audited every hard-delete call site in the codebase; the one real, live, admin-reachable gap on business-critical data was `Lead.bulkDelete` (a permanent `deleteMany` with no recovery path). Fixed: `Lead` gained a `deletedAt` field (mirroring `FileAsset`'s existing pattern), `bulkDelete()` now soft-deletes, a new `bulkRestore()` (`action: "restore"` on the same bulk route) undoes it, `Lead.merge()`'s previously-hard-deleted losing side is now soft-deleted too. Everything else was reviewed and deliberately left as hard delete with stated reasoning: `Pipeline` is already guarded against real data loss (refuses to delete one with opportunities attached); `AutoReplyRule`/`CustomFieldDefinition`/`Tag`/`BrandConfiguration` are configuration, not customer records; `Opportunity`/`Conversation`/`Campaign`/`WhatsAppCampaign`/`Organization`/`User` have no delete path at all today (flagged for future modules, not fixed now); tokens/OTPs/`AuditLog`'s retention sweep are correctly ephemeral/policy-driven. Verified directly against the real database, not just the 725-test unit suite.

**Organization-level data export (§7)** — `POST /api/admin/export` (admin-only) enqueues an async job (reusing RC-3's own MongoDB-backed scheduler, never a new queue) that gathers leads/activities/tasks/opportunities/conversations/campaigns/automation-definitions/payment-history/org-config into one portable JSON file, uploaded via the existing `fileStorageService` (`FileCategory: "EXPORT"`, `visibility: "private"`, tenant-stamped). `GET /api/admin/export/[id]` returns status + a 5-minute signed download URL once complete. Tenant isolation is enforced by construction (`DataExportRequest` and `FileAsset` both carry `tenantScopePlugin` — a cross-tenant id resolves as not-found, never a distinguishing 403) — **verified directly with two real organizations**: Organization A's export completed with a real download URL; the identical request id queried as Organization B returned not-found. **A real bug found during this verification**: `AuditLog`'s Mongoose schema keeps its own hand-maintained `entityType` enum, separate from the TypeScript union — adding the new entity type to the TS side alone silently failed every audit write for it (the model's own comment already disclosed this exact failure mode had happened three times before). Fixed, and the existing regression test that guards against it (`auditLog.model.unit.test.ts`) updated.

**Operator-level tenant restore tooling, deliberately not a one-click button (§8)** — the mission's own explicit instruction: don't build a dangerous unattended tenant restore. `scripts/db/restoreTenantLeadsFromExport.ts` restores Leads + Conversations from an export file into a target organization (dry-run by default, idempotent upserts on natural keys, `organizationId` forcibly the target org regardless of what the export file claims, cross-id remapping verified to keep `conversation.leadId` pointed at the real new lead id after import) — verified end-to-end against the real database including the idempotent-rerun case. Tasks/Opportunities/Campaigns/automation/Payments are explicitly and honestly NOT automated (required references to Users/Pipelines this restore has no safe way to invent, or external-provider-authoritative data that must never be blindly reinserted) — documented as a real manual operator procedure per entity instead of a script that would either crash or silently write broken data.

**Credential & encryption-key recovery documentation (§9)** — audited all four AES-256-GCM encryption modules (tenant credentials, webhook secrets, calendar tokens, MFA secrets): confirmed no key-versioning/rotation mechanism exists today (a lost or rotated key permanently strands everything it encrypted — there is no "restore from backup" for a key, since it's deliberately never stored where the DB backup reaches), confirmed decryption fails loudly on a wrong key (never silently returns garbage), and documented the out-of-band key-preservation procedure reusing RC-4's own Vercel-env-var secret architecture. Also documents exactly which session/token state must NOT be restored after a security incident (RefreshToken, TrustedDevice, PasswordResetToken/EmailVerificationToken/MfaEmailOtp).

**Queue/automation replay-safety classification (§10)** — every one of the 11 registered scheduler job types classified SAFE TO REBUILD / MUST RECOVER / MUST NOT REPLAY AUTOMATICALLY. The clearest finding: `whatsapp_campaign.send_message` checks only the parent campaign's status before calling the real WhatsApp send API — never the individual `Message.status` — meaning a stale restored queue row WOULD re-send a real message to a real recipient if replayed unattended. `webhook.deliver`/`notification.deliver` share the same gap. `payments.reconcile` confirmed genuinely read-only (never charges/captures). Also documents the `WorkflowRun` stale-snapshot exposure beyond the job queue itself, and per-provider reconciliation requirements (Payments/WhatsApp/Calendar/Email/Storage/AI) since a DB restore never rewinds an external system.

**Backup monitoring, reusing the existing pipeline (§11)** — new `BackupLog` collection (system-level, not tenant-scoped) records every real backup attempt; `db:backup`/`db:verify-backup` now report failures immediately through RC-3's own `errorTrackingService` (never a second notification platform); a new self-rescheduling `backup.check_freshness` job (every 6 hours) alerts when the most recent backup is missing, failed, or older than 26 hours. Verified directly: ran a real backup, confirmed a `success` log entry with real size/duration; simulated a failure and confirmed both the immediate alert and the subsequent freshness check correctly detected it.

**Incident response (§12)**, grounded in real, tested capabilities, not aspirational ones. **A genuinely load-bearing gap found and fixed**: the only pre-existing session-revocation mechanism (`sessionService.revokeAllOtherSessions`) was self-service only — there was no way for an admin to force-end a *different*, potentially-compromised user's sessions at all, meaning the "compromised admin account" incident response procedure this section was asked to document would otherwise have been pure theory. New `POST /api/admin/users/[id]/revoke-sessions` (admin-only, with an explicit manual `organizationId` check since `User` isn't `tenantScopePlugin`-scoped) + `sessionService.adminRevokeAllSessions()` — verified directly: created real active sessions for a real user, confirmed all showed `revokedAt` set immediately after calling the new admin path. Two other real gaps found and honestly disclosed rather than fixed under time pressure inside a DR-focused pass: no admin-triggered force-password-reset or MFA-disable route exists yet, and `authService.listActiveStaff()` lists users platform-wide with **no `organizationId` filter at all** — a real cross-tenant finding, flagged as a remaining risk below, not silently patched as a drive-by.

**Recovery mode — evaluated, and deliberately minimal (§13–§14)**: a full maintenance-mode platform (DB-backed flag, admin UI toggle) was evaluated and NOT built — the main danger it would prevent (writes landing on a half-verified restore) is already prevented architecturally by this pass's own restore-to-isolated-environment-first convention and `restoreDatabase.ts`'s hard refusal to overwrite the live database. What WAS built: a single `MAINTENANCE_READ_ONLY_MODE` env var, checked in `withApiRoute` before any handler runs, refusing every mutating request with 503 except `GET`/`HEAD`/`OPTIONS` and `auth.*`-named routes (an operator must still be able to sign in during an incident) — a real, minimal safety valve for an actively-unfolding incident, reversible in about a minute. 5 new unit tests. New `DR_RUNBOOK.md` §13 is the consolidated what-failed/at-risk/which-backup/how-to-restore/how-to-verify/how-to-reconcile/when-to-reopen flow; §15 is the full post-restore checklist across every module (DB, tenant isolation, auth, CRM, WhatsApp, automation, payments, integrations, files, analytics, billing, audit logs, queue/worker, health checks).

**Migration/backfill failure recovery documented (§16)** — audited `lib/db/migrations/index.ts` (real transactional safety via `session.withTransaction()`, with the honest boundary that only writes threaded through the passed session actually roll back) and all five `scripts/backfill*.ts` scripts (none transactional, all idempotent by filter — proven safe to re-run after a kill mid-way, since each is a filter-then-write pattern rather than a checkpoint mechanism). Documents the specific partial-migration-then-deploy-fails scenario this app's own "additive-only within one deploy window" convention already makes self-limiting.

**Testing — 725 total unit tests (up from 720 at RC-4's close, +5 new)** — the new `MAINTENANCE_READ_ONLY_MODE` behavior (refuses mutations, allows reads and auth routes, allows when unset). Full regression: **725/725 unit tests**, `npx tsc --noEmit` clean, project-wide ESLint clean on every RC-5 file (same 74 pre-existing issues in unrelated marketing-site/earlier-RC files remain, confirmed untouched by this pass), production build succeeding with all three new routes (`/api/admin/export`, `/api/admin/export/[id]`, `/api/admin/users/[id]/revoke-sessions`) compiling correctly, a real client-bundle secret scan (clean), and **131/131 E2E specs** (130 passed on the first full run, the one failure — an unrelated Assignment Rule panel test — confirmed flaky/pre-existing by passing cleanly in isolated re-run, not a regression from this pass; the full tenant-isolation and RBAC suites — the areas most relevant to this pass's new routes — passed cleanly on both runs). Every real backup/restore/export/reconciliation/session-revocation claim above was additionally verified directly against the real local MongoDB replica set, not just through the automated suite.

**Remaining, disclosed risks for a future RC pass:** the `whatsapp_campaign.send_message`/`webhook.deliver`/`notification.deliver` job types have no idempotency guard against a replayed queue row (§10) — real hardening work, deliberately not attempted this pass (classification and a documented manual-triage procedure were the RC-5 mandate; retrofitting idempotency into RC-3's own job handlers is its own careful pass). No admin-triggered force-password-reset or MFA-disable route exists (§12.2) — interim CLI-based procedure documented. `authService.listActiveStaff()`/`userRepository.listActive()` has no `organizationId` filter — a real cross-tenant issue in the existing (pre-RC-5) admin staff-directory route, found while researching incident response, small and well-understood but out of RC-5's own explicit backup/DR scope to fix as a drive-by. No `keyVersion` field exists for the four encryption-at-rest keys (§9.3) — rotation today requires a full coordinated re-encryption migration, not a config change. Calendar/Email provider reconciliation after a restore has no automated tooling (§10.4), documented as manual/informational-only. Production MongoDB Atlas backup configuration itself (Cloud Backup enablement, PITR, retention) is REQUIRES PRODUCTION PROVIDER CONFIGURATION — this pass built and verified the tooling and the drill procedure, not a real Atlas account.

**RC-5 (Backup, Restore & Disaster Recovery) is complete against its own approved scope.** Per this pass's own explicit instruction, RC-6 was not started.

---

## RC-4 — Deployment & Production Infrastructure (Production Hardening Release Candidate)

**Goal:** the mission's own words — audit the actual deployment architecture (package.json, Next.js/middleware/vercel.json/cron config, env vars, MongoDB config, queue/worker/Redis, storage, observability, CI, migrations) before changing anything; determine what THIS application actually needs, never add infrastructure "because it's considered enterprise." RC-1/RC-2/RC-3 were complete before this pass began.

**The single most severe finding, found before any code change: the application was never actually in version control.** This repository's `git log` only ever covered the pre-CRM marketing-site landing page — `app/api`, `app/admin`, `lib/services`, `lib/db`, every `config/*.ts` file (≈1,200 files, the entire Business OS built across Phases 0–8 and RC-1–RC-3) had accumulated across many sessions without ever being `git add`ed. A deployment sourced from this repository's own `main` branch would have shipped only the marketing site. Confirmed with the user before acting (a decision of this scope — rewriting the effective contents of the deployed repository — isn't a default to assume silently) and committed the full application as a single baseline catch-up commit, verified clean of secrets first (`.gitignore` already excluded every `.env*`/`.storage-uploads/`; a targeted regex scan for real API-key/token/private-key shapes and a broader `secret|password|apiKey|token: "..."` sweep both came back clean — the only matches were obviously-fake E2E test fixture values). **Not pushed** — that remains the user's own explicit action.

**Deployment target confirmed as Vercel, not migrated away from** — `vercel.json`'s cron entry, `middleware.ts`'s Edge-runtime split, every outbound provider timeout (RC-3), and every route's own bounded execution model all already assume Vercel's serverless request/response shape. No concrete incompatibility was found; RC-3's own Redis/BullMQ-vs-MongoDB-scheduler decision was re-verified, not re-litigated.

**Real bugs found and fixed, not just documented:**

1. **A connection-cache poisoning bug**: `lib/db/connection.ts` cached a REJECTED `mongoose.connect()` promise forever — one transient MongoDB outage (a network blip, an Atlas failover) would permanently break every future request for the rest of that serverless instance's lifetime, never attempting to reconnect even after MongoDB recovered. Fixed to clear the cached promise on failure so the next call genuinely retries; regression-tested against a real concurrent-caller scenario (two callers sharing one in-flight attempt, never two separate `connect()` calls).
2. **No `autoIndex` control** — Mongoose's own default (`true`) was running `createIndex()` for every one of 48 models on every single connection, meaning every cold start in production: real, avoidable latency plus lock-contention risk against a large collection, and exactly what the mission's own "do NOT automatically create destructive indexes/migrations during every production boot" instruction rules out. Disabled in production; index creation is now a deliberate, operator-triggered step (`npm run db:sync-indexes`, a new script importing all 48 models and calling `syncIndexes()` on each — live-verified against the real local MongoDB replica set). Found and fixed 6 genuinely duplicate index declarations along the way (a field's own `unique: true` plus a redundant explicit `.index()` call for the same key, across `User`/`Organization`/`RefreshToken`/`TrustedDevice`/`EmailVerificationToken`/`PasswordResetToken`) — Mongoose's own duplicate-index warnings went from 6 to 0.
3. **`runPendingMigrations()` had zero callers anywhere** — a real, idempotent, transactional migration runner existed since the original schema scaffold but nothing ever invoked it. New `npm run db:migrate` (`scripts/runMigrations.ts`) is the real, documented, operator-triggered entry point — deliberately still not wired to run automatically at boot, per the mission's own explicit "separate APPLICATION STARTUP from DATA MIGRATION" instruction.
4. **The scheduler poller had no time budget** — `runDueScheduledJobs()` processes its batch of due jobs SEQUENTIALLY (by design, not a bug), so `DEFAULT_BATCH_SIZE` (20) bounded job COUNT but not WALL-CLOCK duration: a batch of jobs each near their own worst-case provider timeout (up to 30s for an AI call) could take up to ~10 minutes, far past any serverless function's real duration limit — Vercel would kill the invocation mid-batch. Fixed with a real 45-second time budget checked before claiming each new job (not simply raising a timeout — the mission's own explicit warning): the function always finishes whatever job it's currently on, then stops cleanly and leaves the remainder for the next cron tick 5 minutes later, rather than risking a mid-job kill. Both job-draining routes (`/api/cron/run-due-jobs`, `/api/admin/scheduler/run-due-jobs`) now also declare an explicit `maxDuration = 60` — a deployment-plan-independent ceiling — and the two CSV-bulk-import routes (general CRM import, WhatsApp campaign audience import — both row-by-row-validated, capped at 5,000 rows, not bulk inserts) do the same.
5. **`MONGODB_URI` was only "recommended," not "required," in startup validation** — promoted to the required tier: the mission's own CRITICAL-configuration list names "DATABASE" first, and running production traffic against the in-memory fallback is silent, total data loss (nothing persists past a restart, nothing is shared across serverless instances), not merely an insecure-but-functional default the way the six encryption/signing secrets are.
6. **The "local" file storage provider is silently unsafe on Vercel** — `process.cwd()` is read-only outside `/tmp` on a real serverless deployment, and `/tmp` itself isn't shared across instances or durable across invocations; uploads would fail or silently vanish. `lib/startupValidation.ts` now logs a loud startup error whenever `STORAGE_PROVIDER` is unset/`local` in production.
7. **No safety signal for a real send provider active in a Vercel preview/staging deployment** — a new check (`checkPreviewEnvironmentSafety()`, keyed off Vercel's own automatic `VERCEL_ENV=preview` signal, which needs no configuration to get) warns loudly if a preview deployment has a real WhatsApp or Email provider active rather than the safe `console` no-op default — the real, common mistake this guards against is an operator copying production's env wholesale into staging and a staging bug messaging a real customer. Deliberately a warning, not a block — a preview environment legitimately testing against a real vendor SANDBOX account is a real use case this check can't distinguish from a genuine misconfiguration, so it stays loud rather than silently either allowing or blocking.
8. **CI never ran unit tests or a production build** — `.github/workflows/ci.yml` previously gated merges on type-check + lint + an E2E smoke suite only; a change could break either and still merge clean. Added `npm run test:unit` and `npm run build` as real gates, plus a separate (non-blocking-by-default) `dependency-audit` job running `npm audit --audit-level=high` in parallel, so a transitive-dependency advisory can't block unrelated work while staying visible.
9. **`.env.example` covered roughly half of this app's real configuration surface** — Storage (Module 6.2), Calendar (Module 6.3), and Payments (Module 6.4) env vars were entirely undocumented (verified by cross-referencing every `process.env.X` reference across every `config/*.ts` file against the file). Rewritten comprehensively: every real variable documented, grouped by feature area, and classified per the mission's own scheme (`[REQUIRED]`/`[RECOMMENDED]`/`[PLATFORM]`/`[TENANT-AWARE]`/`[PUBLIC]`/`[OPTIONAL]`), with an explicit LOCAL/STAGING/PRODUCTION section.
10. **`tsx` was an undeclared transitive dependency** — every operational script (`createAdminUser.ts`, `resetAdminPassword.ts`, and this pass's own new scripts) has always run via `npx tsx`, which happened to resolve only because some other package pulled it in as an optional peer dependency, never because this app declared it. Added as a real, locked `devDependency`. Also added an `engines.node` field (`>=22.0.0`, matching CI's own pinned Node version) — neither existed before.
11. **`.gitignore`'s own `.env*` pattern was ALSO silently swallowing `.env.example` itself** — found while committing this pass's own rewritten `.env.example` (finding #9 above): a genuinely committed-by-design template with no real secrets in it (see that file's own header) had never actually been tracked, meaning a fresh clone of this repository had zero documentation of what environment variables even exist — directly undermining the mission's own "Fresh Deployment Test" requirement. Fixed with a negated `!.env.example` rule immediately after the exclusion (gitignore evaluates rules in order; the un-ignore only works placed after the rule it overrides).

**New: a real preflight/configuration-check mechanism** (mission §34) — `lib/services/systemHealth/preflightService.ts` is the one shared assessment two consumers use: `npm run preflight` (a CLI gate, exit code 1 if anything critical is unconfigured) and `GET /api/admin/system/preflight` (live visibility on an already-deployed instance, since there's no shell access into a real Vercel deployment). Reports Database/Authentication/Encryption/Queue/Cron/Workers/Storage/Observability as ok/warning/critical, plus every tenant-level optional integration (WhatsApp/Email/AI/Payments/Calendar) as configured-or-not — **never blocking the platform for an unconfigured tenant-level integration**, the mission's own explicit instruction. "Workers" reports informational, not pass/fail — there is no persistent worker process in this app's real architecture to be "reachable"; Vercel Cron directly drains the MongoDB-backed scheduler, and saying so explicitly is more honest than silently omitting a category the mission named.

**Docker/Kubernetes: evaluated, neither adopted** — documented explicitly in `RUNBOOK.md` §3 rather than left implicit. Docker would containerize a web tier Vercel already builds and runs directly from source, for a persistent-worker use case this app doesn't have (RC-3's own architecture decision). Kubernetes solves orchestration problems (many long-running containers, custom autoscaling, service-mesh networking) this one-Next.js-app-plus-a-handful-of-scripts deployment doesn't have. Revisit only if a future module genuinely needs a persistent, always-running process no serverless function can host.

**New `RUNBOOK.md`** — architecture map, deployment steps, the Docker/K8s decision, environment tiers (with staging-specific safety notes), migration/index-sync procedure, health/readiness/preflight verification commands, rollback procedure (application vs. database-migration rollback are explicitly NOT the same thing — application rollback never automatically reverses a data migration), and a common-failures table. No real secrets anywhere in it.

**External API reality matrix** — classified CODE READY / CONFIGURED (in this environment) / LIVE VERIFIED, not asserted from code existing alone:

| Integration | Code Ready | Configured (this env) | Live Verified |
|---|---|---|---|
| WhatsApp — Meta Cloud API | ✅ | ❌ (`console` default) | Partial — real Graph API rejection path proven (Module 2.3); no real send ever completed (no real Meta App in this environment) |
| Email — Postmark | ✅ | ❌ | ❌ |
| AI — Anthropic | ✅ | ❌ | ✅ — real `401` from Anthropic's live API with an intentionally invalid key, repeated across three modules (5.1/5.2/5.3) |
| AI — OpenAI / Gemini | ✅ | ❌ | ❌ — adapters are real (fetch-based, same shape as Anthropic's), never independently live-tested |
| Payments — Razorpay | ✅ | ❌ | ✅ — real rejection confirmed against Razorpay's live API with test keys (Module 6.4) |
| Payments — Stripe / Cashfree | ✅ | ❌ | ❌ |
| Payments — PhonePe / PayPal | Scaffold only (`PaymentProviderNotImplementedError`) | ❌ | N/A |
| Calendar — Google / Microsoft / Zoom | ✅ | ❌ | ❌ — OAuth apps never registered in this environment; "Connect" correctly redirects and reports the honest "missing env vars" state |
| Storage — AWS S3 | ✅ (official SDK, not hand-rolled) | ❌ | ❌ |
| Storage — Cloudinary | ✅ (disclosed gap: no private-URL signing) | ❌ | ❌ |
| Storage — local (dev only — not production-safe on Vercel, see fix #6 above) | ✅ | ✅ (dev default) | ✅ — real upload/download/delete cycle in a real browser |

**Testing — 18 new unit tests** (720 total, up from 702) — connection-retry/concurrency, the autoIndex/duplicate-index fix, promoted `MONGODB_URI` severity, ephemeral-storage and preview-environment safety checks, the scheduler's own time-budget early-exit (a real fake-clock simulation proving a 4-job batch that would exceed budget correctly processes 3 and leaves the 4th `pending` for the next tick), and the preflight service's own category logic. No new E2E specs this pass (RC-4 is infrastructure/config, not new user-facing behavior) — the full existing 131-spec suite re-run clean, including cross-tenant isolation and authentication regression. Full regression: **720/720 unit tests, 131/131 E2E specs**, `npx tsc --noEmit` clean, project-wide ESLint clean on every file this pass touched (same 74 pre-existing issues in unrelated marketing-site components remain, disclosed and untouched — RC-2's own established scope boundary), `npm ci` deterministic install verified, production build succeeding **against a completely empty environment** (verified directly — zero env vars set, matching CI's own clean state), `npm audit` 0 vulnerabilities, and a real client-bundle secret scan (every server-only secret with a real local value confirmed absent from `.next/static/`, methodology verified by confirming a known-public value DOES appear first).

**Remaining, disclosed risks for a future RC pass:** file uploads route through this app's own Next.js API function rather than a presigned direct-to-storage flow — Vercel's platform-level serverless request-body ceiling is well below this app's own configured per-category limits (up to 50MB for VIDEO/EXPORT), so a large upload may be rejected by the platform itself before this app's own validation ever runs; the real fix (presigned browser-to-S3 uploads) is a genuine architectural change touching every upload call site, not attempted this pass. CSV import (both the general CRM and WhatsApp-campaign-audience routes) remains a synchronous, row-by-row operation at its existing 5,000-row cap — now duration-ceilinged honestly (`maxDuration=60`) rather than silently relying on a platform default, but not converted to an async background job; worth revisiting if that cap is ever raised significantly. Most external vendor integrations remain CODE READY but not LIVE VERIFIED beyond a rejection-path round-trip (see the matrix above) — each requires its own real vendor account, the same disclosed constraint every prior RC pass touching these integrations has carried.

**RC-4 (Deployment & Production Infrastructure) is complete against its own approved scope.** Per this pass's own explicit instruction, RC-5 was not started.
...
## RC-1 — Release Candidate Stabilization (historical — pre-blueprint, unrelated to the RC-1–RC-7 Production Hardening track above)

**RC-8 disambiguation note (2026-08-05):** this "RC-1" and the
"RC-1 — Authentication & Identity" entry near the top of this file are
two different, unrelated things that happen to share a name — this one
predates the 35-module Business OS blueprint and the later Production
Hardening Release Candidate track (RC-1 through RC-7) entirely. Kept
here unedited as the real historical record; see
[`docs/README.md#source-of-truth`](docs/README.md#source-of-truth) for
the full explanation of this naming collision and which "RC-1" a given
cross-reference actually means.
